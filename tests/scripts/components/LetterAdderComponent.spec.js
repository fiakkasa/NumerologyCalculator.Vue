...
describe('LetterAdderComponent', function () {
    ...
    function mountComponent(initialProps = {}, services = {}, eventSpies = {}) {
        ...
        const app = Vue.createApp({
            render() {
                ...
                return Vue.h(LetterAdderComponent, vnodeProps);
            }
        });

        app.provide('uiService', services.uiService || {});
        app.provide('letterCalculatorService', services.letterCalculatorService || {});
    ...
    it('calls services, updates result and renders when valid normalized input provided', async function () {
        ...
        const calcResult = '3';
        const calcSteps = [{ equation: '(A: 1) + (B: 2)', numberOfCharacters: 2 }];
        ...
        expect(stepEl.textContent).toBe('(A: 1) + (B: 2)');
